...
// Editor.jsx - FULLY RESPONSIVE WITH ALL ORIGINAL FEATURES
import React, { useState, useEffect, useRef } from "react";
import { Rnd } from "react-rnd";
import { ReactSortable } from "react-sortablejs";
import { useParams } from "react-router-dom";
...
function Editor() {
  ...
  const addImageLayerFromFile = (file) => {
    ...
    const newLayer = {
      ...
      fit: "contain",
      locked: false,
      // ✅ PERSPECTIVE PROPERTIES (Original)
      perspective: 0,
      rotateX: 0,
      rotateY: 0,
      rotateZ: 0,
  ...
  const addPrintAreaToCanvas = (printArea) => {
    const newLayer = {
      ...
      border: true,
      locked: false,
      // ✅ PERSPECTIVE PROPERTIES FOR PRINT AREA TOO
      perspective: 0,
      rotateX: 0,
      rotateY: 0,
      rotateZ: 0,
        ...
        <div className="flex items-center justify-between p-3 lg:p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-gray-700 rounded-lg p-1">
              <button onClick={undo} className="p-2 rounded hover:bg-gray-600" title="Undo (Ctrl+Z)">⤺</button>
              <button onClick={redo} className="p-2 rounded hover:bg-gray-600" title="Redo (Ctrl+Y)">⤻</button>
            </div>

            {/* Export Buttons */}
            <button onClick={() => exportAsImage("png")} className="px-3 py-2 rounded bg-emerald-600 hover:bg-emerald-700" title="Export PNG (Ctrl+E)">
              PNG
            </button>
            <button onClick={() => exportAsImage("jpg")} className="px-3 py-2 rounded bg-amber-600 hover:bg-amber-700" title="Export JPG">
              JPG
            </button>
            ...
            <div className="hidden sm:flex items-center bg-gray-700 p-1 rounded">
              <button onClick={handleZoomOut} className="px-2 py-1 rounded hover:bg-gray-600" title="Zoom out (Ctrl+-)">−</button>
              <div className="px-2 text-sm">{Math.round(scale * 100)}%</div>
              <button onClick={handleZoomIn} className="px-2 py-1 rounded hover:bg-gray-600" title="Zoom in (Ctrl+= / Ctrl++)">+</button>
              <button onClick={handleZoomReset} className="px-2 py-1 rounded hover:bg-gray-600" title="Reset zoom (Ctrl+0)">reset</button>
            </div>
          </div>
        </div>

        {/* Print Area Select Dropdown */}
        {showPrintareaSelect && (
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`${activePanel === 'layers' ? 'block' : 'hidden'} lg:block w-full lg:w-64 bg-gray-800 p-4 space-y-4 overflow-auto`}>
          ...
          >
            {layers.map((layer) => (
              <div
                ...
                className={`w-full flex items-center justify-between p-3 rounded cursor-pointer transition ${selectedLayerId === layer.id
                  ? "bg-blue-600 text-white"
                  : "bg-gray-700 hover:bg-gray-600 text-gray-200"
                  }`}
              >
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={addNewTextLayer} className="flex-1 py-2 bg-green-600 hover:bg-green-700 rounded shadow transition">
              + Text
            </button>
            <button onClick={addImageLayerButton} className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-700 rounded shadow transition">
              + Image
            </button>
          </div>
        </div>
        ...
        <div className={`${activePanel === 'properties' ? 'block' : 'hidden'} lg:block w-full lg:w-64 bg-gray-800 p-4 lg:p-6 space-y-4 lg:space-y-6 overflow-auto`}>
          ...
          <button onClick={duplicateLayer} disabled={!selectedLayer} className={`w-full py-2 rounded shadow mb-4 transition ${selectedLayer ? "bg-yellow-600 hover:bg-yellow-700" : "bg-gray-600 cursor-not-allowed"}`}>
            Duplicate Layer
          </button>
          ...
          {selectedLayer ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 items-center">
                <div className="text-gray-400 font-medium">Layer Name</div>
                <button onClick={toggleLockSelected} className={`ml-auto px-2 py-1 rounded ${selectedLayer.locked ? "bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}>
                  {selectedLayer.locked ? "Unlock" : "Lock"}
                </button>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="range" min="0" max="1" step="0.01" value={selectedLayer.opacity ?? 1}
                    onChange={(e) => updateLayer(selectedLayer.id, { opacity: parseFloat(e.target.value) })} className="flex-1" />
                  <div className="w-12 text-right">{Math.round((selectedLayer.opacity ?? 1) * 100)}%</div>
                </div>
              </div>
              ...
              {selectedLayer.type === "image" && (
                <>
                  ...
                  <div>
                    <p className="text-gray-400 font-medium">Fit Mode</p>
                    <select value={selectedLayer.fit || "contain"} onChange={(e) => updateLayer(selectedLayer.id, { fit: e.target.value })}
                      className="w-full p-2 rounded bg-gray-700 text-white">
                      <option value="contain">Contain</option>
                      <option value="cover">Cover</option>
                      <option value="fill">Fill</option>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" checked={selectedLayer.enablePerspective || false} onChange={(e) => {
                          const checked = e.target.checked;
                          updateLayer(selectedLayer.id, { enablePerspective: checked });
                          setShowGrid(checked);
                        }} className="sr-only peer" />
                        <div className="w-11 h-6 bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-purple-600"></div>
                      </label>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="mb-3">
                          ...
                          <input type="range" min="0" max="1000" step="10" value={selectedLayer.perspective || 0}
                            onChange={(e) => updateLayer(selectedLayer.id, { perspective: parseInt(e.target.value) || 0 })} className="w-full" />
                          <div className="flex justify-between text-xs text-gray-400">
                            <span>0px</span>
                            <span>500px</span>
                            <span>1000px</span>
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-3">
                          <div>
                            ...
                            <input type="number" className="w-full p-2 rounded bg-gray-700 text-white" value={selectedLayer.rotateX || 0}
                              onChange={(e) => updateLayer(selectedLayer.id, { rotateX: parseInt(e.target.value) || 0 })} min="-180" max="180" />
                          </div>
                          <div>
                            <p className="text-gray-400 text-sm">Rotate Y</p>
                            <input type="number" className="w-full p-2 rounded bg-gray-700 text-white" value={selectedLayer.rotateY || 0}
                              onChange={(e) => updateLayer(selectedLayer.id, { rotateY: parseInt(e.target.value) || 0 })} min="-180" max="180" />
                          </div>
                          <div>
                            <p className="text-gray-400 text-sm">Rotate Z</p>
                            <input type="number" className="w-full p-2 rounded bg-gray-700 text-white" value={selectedLayer.rotateZ || 0}
                              onChange={(e) => updateLayer(selectedLayer.id, { rotateZ: parseInt(e.target.value) || 0 })} min="-180" max="180" />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-3">
                          <div>
                            ...
                            <input type="number" className="w-full p-2 rounded bg-gray-700 text-white" value={selectedLayer.skewX || 0}
                              onChange={(e) => updateLayer(selectedLayer.id, { skewX: parseInt(e.target.value) || 0 })} min="-45" max="45" />
                          </div>
                          <div>
                            <p className="text-gray-400 text-sm">Skew Y</p>
                            <input type="number" className="w-full p-2 rounded bg-gray-700 text-white" value={selectedLayer.skewY || 0}
                              onChange={(e) => updateLayer(selectedLayer.id, { skewY: parseInt(e.target.value) || 0 })} min="-45" max="45" />
                          </div>
                        </div>
                        ...
                        <div>
                          <p className="text-gray-400 font-medium">Transform Origin</p>
                          <select value={selectedLayer.transformOrigin || "center center"} onChange={(e) => updateLayer(selectedLayer.id, { transformOrigin: e.target.value })}
                            className="w-full p-2 rounded bg-gray-700 text-white">
                            <option value="center center">Center</option>
                            <option value="top left">Top Left</option>
                            <option value="top center">Top Center</option>
                  ...
                  {/* ✅ PERSPECTIVE CONTROLS FOR PRINT AREA (COMPLETE) */}
                  <div className="pt-2 border-t border-gray-700">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-gray-300 font-medium">Perspective</h3>
                      <label className="relative inline-flex items-center cursor-pointer">
              ...
              {selectedLayer.type === "printarea" && (
                <>
                  <div className="pt-2 border-t border-gray-700">
                    <p className="text-gray-400 font-medium mb-2">Print Area Image</p>
                    <input ref={printAreaFileInputRef} type="file" accept="image/*" onChange={handlePrintAreaImageUpload} className="hidden" />
                    {!selectedLayer.hasImage ? (
                      <div className="space-y-3">
                        <button onClick={triggerPrintAreaImageUpload} className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 rounded-lg font-medium transition flex items-center justify-center gap-2">
                          Upload Image
                        </button>
                      ...
                      <div className="space-y-4">
                        <div className="relative bg-gray-900 rounded-lg p-3">
                          ...
                          <div className="flex justify-between mt-3">
                            <button onClick={triggerPrintAreaImageUpload} className="py-2 px-4 bg-blue-600 hover:bg-blue-700 rounded transition">
                              Replace Image
                            </button>
                            <button onClick={removePrintAreaImage} className="py-2 px-4 bg-red-600 hover:bg-red-700 rounded transition">
                              Remove Image
                            </button>
                          </div>
                        </div>
                        <div>
                          <p className="text-gray-400 font-medium">Image Fit</p>
                          <select value={selectedLayer.fit || "cover"} onChange={(e) => updateLayer(selectedLayer.id, { fit: e.target.value })}
                            className="w-full p-2 rounded bg-gray-700 text-white">
                            <option value="cover">Cover (Fill entire area)</option>
                            <option value="contain">Contain (Fit inside)</option>
                            <option value="fill">Fill (Stretch)</option>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-2 mb-3">
                          <div><input type="number" className="w-full p-2 rounded bg-gray-700 text-white" value={selectedLayer.rotateX || 0}
                            onChange={(e) => updateLayer(selectedLayer.id, { rotateX: parseInt(e.target.value) || 0 })} placeholder="Rotate X" /></div>
                          <div><input type="number" className="w-full p-2 rounded bg-gray-700 text-white" value={selectedLayer.rotateY || 0}
                            onChange={(e) => updateLayer(selectedLayer.id, { rotateY: parseInt(e.target.value) || 0 })} placeholder="Rotate Y" /></div>
                          <div><input type="number" className="w-full p-2 rounded bg-gray-700 text-white" value={selectedLayer.rotateZ || 0}
                            onChange={(e) => updateLayer(selectedLayer.id, { rotateZ: parseInt(e.target.value) || 0 })} placeholder="Rotate Z" /></div>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {/* Layer Order Controls */}
              <div className="flex gap-2 pt-4 border-t border-gray-700">
                <button onClick={() => bringForward(selectedLayer.id)} className="flex-1 py-2 bg-gray-600 hover:bg-gray-500 rounded transition">
                  Bring Forward
                </button>
                <button onClick={() => sendBackward(selectedLayer.id)} className="flex-1 py-2 bg-gray-600 hover:bg-gray-500 rounded transition">
                  Send Backward
                </button>
              </div>
              ...
              {/* Delete Layer */}
              <div className="pt-2">
                <button onClick={() => removeLayer(selectedLayer.id)} className="w-full py-2 bg-red-600 hover:bg-red-700 rounded transition">
                  Delete Layer
                </button>
              </div>
            </div>
