...
function Topbar({ onMenuClick }) {
  ...
  useEffect(() => {
    ...
    const updateProfileFromStorage = () => {
      try {
        const user = JSON.parse(localStorage.getItem("user"));
        const newImage = user?.profileImage?.url;
        console.log("Profile image updated to:", newImage); // Debug log
    ...
    const handleCustomEvent = (e) => {
      console.log("Custom event received:", e.type); // Debug log
      updateProfileFromStorage();
    };
    ...
    // Polling method as backup (check every 500ms)
    const interval = setInterval(updateProfileFromStorage, 500);
